...
{
    $salida=array();
    $query="SELECT motivo_credito.IDMotivo,
    motivo_credito.nombre,
    motivo_credito.estado
    FROM motivo_credito
    WHERE Estado='activo'";
    $res=mysql_query($query,conectar::con());
    while($row=mysql_fetch_assoc($res))
...
QUERY;
	...
	elseif ($_GET["action"]=="editarDatos")
	{
       ...
       $Folio     				= $_POST["Folio"];
       $Folio_nota_credito     = $_POST["Folio_nota_credito"];
       ...
       $fecha 		 = date_create($FechaFacturacion);
