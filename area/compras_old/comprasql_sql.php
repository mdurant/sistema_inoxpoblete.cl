...
try
{
  ...
  if($_GET["action"] == "list")
  {
    ...
    $where="WHERE $dat (
      suppliers.Suppliers LIKE '$product%' 
      or 
      empresa.RazonSocial LIKE '%$product%'
      or
      ecompra.contador LIKE '$product%'
      or
      suppliers.RUT = '$product'
      or
      ecompra.folio_factura = '$product'
      ) and ecompra.estado = 'activo' $radio";
    $limit="LIMIT $jtStartIndex,$jtPageSize";

    $orden = "order by $jtSorting";
...
QUERY;
  ...
  else if($_GET["action"] == "delete")
  {
    ...
    $delete=<<<QUERY
    UPDATE  ecompra SET  estado =  'inactivo' WHERE  ecompta.estado ='$IdEFactura';
QUERY;
