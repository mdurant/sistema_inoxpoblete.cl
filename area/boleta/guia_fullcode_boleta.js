...
$(".cod").on('keyup',function(){
	...
	}else
	{
		...
		var datasrl =$("#form_tbl").serialize();
			...
			type:"POST",
			dataType:"json",
			//url:"http://localhost/developer/crud/factura/factura_json.php",
			url:"boleta_json.php",
			data:datasrl,
			beforeSend: function () {
				$("#pre-load").show();
...
//TRABAJAR CON EL BTN PARA GUARDAR LOS DATOS
	...
	$("#btn-cotizacion").on('click',function(event){
		...
		if(valores=="valgo")
		{
			...
			type:"POST",
			dataType:"json",
			//url:"http://localhost/developer/crud/factura/insert_factura_sql.php",
			url:"insert_boleta_sql.php",
			data:datasrl,
			beforeSend: function () {
				$("#pre-load").show();
           },
			...
			success: function(response){
					...
					alert("Boleta Creada y generada");
					$(".act").prop("disabled",true); 	
					window.location = 'boletacrud_main.php';
