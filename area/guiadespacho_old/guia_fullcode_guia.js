...
function calcular_total() {
    ...
    $("#tbl_foot tr td:eq(1)").html('<center><small><strong>Total Neto:</strong></small><input class="form-control input-sm neto" type="text" disabled id="neto_guia" name="neto_guia" value="'+Moneda(neto.toFixed())+'"/></center>');
...
$(".cant").keypress(function(event) {
    ...
    $("#btnguardar").on('click', function(event) {
        ...
        if ($("#contador").val() == "" || $("#fecha_solicitud").val() == "" || $("#jefe").val() == "" || $("#proyecto").val() == "" || $("#solicitante").val() == "") {
            alert("Rellene campos nescesarios, favor revisar la interfaz");
        } else {
            ...
            $.ajax({
                ...
                type: "POST",
                dataType: "json",
                url: "insert_guia_sql.php",
                data: datasrl,
                success: function(response) {
                    alert(data.neto_guia);
                    alert("Guía Despacho creada con Éxito!");
                    alert(response);
                    window.location = 'guiacrud_main.php';
